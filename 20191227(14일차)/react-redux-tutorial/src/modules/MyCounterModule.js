...
export const munusAction = createAction(MINUS);
...
// 각각의 함수들이 action값에 의해 선택적으로 호출되고, 여기서 새로운 initialState 구조의 데이터를 리턴하면 그 내용이 화면에 자동으로 반영된다.
const MyCountReducer = {
    // 주의할점! 'MyCounterModule/PLUS'를 담아둔 변수를 사용할 경우 [] 를 사용해야함. 그렇지않으면 그냥 json 형태가 되어버림.
    [PLUS]: function(state, action){
        ...
        // !!!!!!!!!return 이 중요. 여기서 const initialState 값과 동일한 패턴으로 return해주면 화면에 자동으로 반영됨
